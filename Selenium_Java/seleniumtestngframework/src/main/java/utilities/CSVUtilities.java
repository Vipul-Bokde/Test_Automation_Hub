// CSVUtilities.java (Original Version - No Sheet Handling)
package utilities;
...
public class CSVUtilities implements AutoCloseable {
	...
	private static final String ENCRYPTED_FILE_PATH = ConfigReader.getProperty("outputPath");
	private static final String TEMP_FILE_PATH = ConfigReader.getProperty("tempPath");
	...
	public Map<String, String> readData() {
		...
		try (CSVReader reader = new CSVReader(new FileReader(TEMP_FILE_PATH))) {
			...
			String[] valueRow = rows.get(1); // Assumes only one data row
