...
def check_checkbox(self, locator_type, locator):
    ...
    if not is_checked:
        ...
        #self.driver.execute_script('arguments[0].click()', (locator_type, locator))
        mouse.click_action_on_element(self, locator_type, locator)
...
def verify_ischecked(self, locator_type, locator):
    """Verify whether the checkbox is checked or not"""
    is_checked = locators.check_selected(self, locator_type, locator)
    return is_checked
...
def get_options(self, locator_type, locator):
    ...
    for option in options:
        optionsList.append(option.text)
    return optionsList
...
def verify_status(self, locator_type, locator):
    status = locators.get_attribute_value(self, locator_type, locator, 'title')
    return status 